...
bot.on("message", async message =>{
    ...
    // PREFIX COMMAND - This requires the quick.db package line 4
    let fetched = await db.fetch(`prefix_${message.guild.id}`);
    if (fetched ===null) prefix = `>`;// If not set, fetched will be `null`
    else prefix = fetched; // Although, if its defined, set fprefix to what we fetched.
